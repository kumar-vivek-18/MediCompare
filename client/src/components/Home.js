...
const Home = () => {
  return (
    <div className='max-md:mt-[120px] bg-[#F1FAFE] mt-[80px] w-screen flex items-center justify-center'>
        ...
        <div className='flex flex-row gap-[20px] justify-between items-center mb-[10px]'>
           {
            Companies.map((company) => (
                <Link to={company?.link} key={company.id} className='flex flex-1 flex-row items-center justify-center border-l-[1px] border-r-[1px] hover:scale-105 transition-all duration-100 cursor-pointer'>
                    <div className='flex flex-col items-center justify-center '>     
                         <img src={company.logo} alt={company.name} className='w-[100px] h-[40px] object-cover ' />
                        {/* <h3>{company.name}</h3> */}
